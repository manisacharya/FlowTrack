function appState() {
  return {
    ...
    async toggleTask(task) {
      await fetch('./api/tasks?id=' + task.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ completed: task.completed == 1 ? 0 : 1 }) });
      await Promise.all([this.loadTasks(), this.loadMetrics(), this.loadCalendarTasks()]);
    },
    ...
    async markHabit(id) {
      await fetch('./api/mark_habit?id=' + id, { method: 'POST' });
      await Promise.all([this.loadHabits(), this.loadMetrics()]);
    },
    ...
    async toggleMark(habitId, ymd) {
      ...
      if (this.isMarked(habitId, ymd)) {
        await fetch(`./api/unmark_habit?id=${habitId}&d=${ymd}`, { method: 'POST' });
      } else {
        await fetch(`./api/mark_habit?id=${habitId}&d=${ymd}`, { method: 'POST' });
      }
    ...
    async loadCalendarTasks() {
      ...
      const res = await fetch(`./api/tasks?start_due=${start}&end_due=${end}`);
    ...
    async addRoutine(kind) {
      ...
      await fetch('./api/routines', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ kind, text, sort }) });
      await this.loadRoutines();
    },
    ...
    async editRoutine(item) {
      ...
      if (text === null) return;
      await fetch('./api/routines?id=' + item.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text }) });
      await this.loadRoutines();
    },
